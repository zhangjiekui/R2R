...
class XinferenceEmbeddingProvider(EmbeddingProvider):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        if not provider:
            raise ValueError(
                "Must set provider in order to initialize `XinferenceEmbeddingProvider`."
            )
        if provider != "xinference":
            raise ValueError(
                "XinferenceEmbeddingProvider must be initialized with provider `xinference`."
            )
        ...
        if config.rerank_model:
            if url := config.rerank_url or os.getenv("XINFERENCE_API_BASE"):
                ...
                raise ValueError(
                    "XinferenceEmbeddingProvider requires a valid reranking API url to be set via `embedding.rerank_url` in the r2r.toml, or via the environment variable `XINFERENCE_API_BASE`."
                )
    ...
    def _get_embedding_kwargs(self, **kwargs):
        embedding_kwargs = {
            ...
            "model": f"xinference/{self.base_model}",
    ...
    async def async_get_embedding(
        ...
        **kwargs,
        ...
        if stage != EmbeddingProvider.Step.BASE:
            raise ValueError(
                "XinferenceEmbeddingProvider only supports search stage."
            )
    ...
    def get_embedding(
        ...
        **kwargs,
        ...
        if stage != EmbeddingProvider.Step.BASE:
            raise ValueError(
                "Error getting embeddings: XinferenceEmbeddingProvider only supports search stage."
            )
    ...
    def rerank(
        ...
        if self.config.rerank_model is not None:
            if not self.rerank_url:
                raise ValueError(
                    "Error, `rerank_url` was expected to be set inside XinferenceEmbeddingProvider"
                )
            ...
            try:
                ...
                if isinstance(reranked_results, dict):
                    valid_reranked_results = reranked_results.get('results',None)
                    if valid_reranked_results:
                        reranked_results = valid_reranked_results
                ...
                for rank_info in reranked_results:
                    ...
                    copied_result.score = rank_info.get("score",rank_info.get("relevance_score",0.0))
